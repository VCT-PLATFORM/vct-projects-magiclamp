# AGENTS.md — Magic Lamp AI Roster & Rules

> **Mục đích**: Định nghĩa danh sách các Agent chuyên biệt và quy tắc thực thi.

## 1. Đội ngũ Quản trị Nội bộ (Internal Core)

| Agent | Role | Chuyên môn |
|:---|:---|:---|
| **Jen (CoS)** | Chief of Staff | Điều phối tổng, lập kế hoạch, báo cáo Chairman. |
| **ML-HR** | HR Manager | Tuyển dụng Creator/Idol, tính lương, quản lý KPIs. |
| **ML-Finance** | Accountant | Xử lý hóa đơn, đối soát tiền live, báo cáo P&L. |
| **ML-Legal** | Legal Counsel | Hợp đồng bản quyền, HĐ KOL, tuân thủ chính sách sàn. |
| **ML-Admin** | Office Manager | Quản lý tài sản văn phòng, lịch họp. |
| **ML-Studio** | Studio Manager | Quản trị thiết bị quay, lịch sử dụng Studio & Livestream. |

## 2. Đội ngũ Agency (Production & Performance)

| Agent | Role | Chuyên môn |
|:---|:---|:---|
| **Creative-AI** | Script Writer | Viết kịch bản TikTok/Shorts, lên ý tưởng Campaign. |
| **ML-Account** | Account Manager | Quản trị quan hệ khách hàng, tiếp nhận brief & feedback. |
| **ML-Strategy** | Trend Strategist | Phân tích xu hướng vĩ mô, tối ưu "Công thức triệu view". |
| **Market-Researcher**| Research Specialist | Nghiên cứu đối thủ, Insight khách hàng & Phân tích thị trường. |
| **Brand-Architect** | Brand Strategist | Định vị thương hiệu, Storytelling & Nhận diện cốt lõi. |
| **Media-Planner** | Media Strategist | Lập kế hoạch ngân sách, chọn kênh & Forecast KPIs. |
| **SEO-Specialist** | Growth Specialist | Tối ưu hóa công cụ tìm kiếm, Website traffic & Keywords. |
| **Social-Growth** | Community Manager | Seeding, quản trị cộng đồng & Viral distribution. |
| **Live-Ops** | Livestream Coordinator | Sắp xếp lịch Live, gắn giỏ hàng, tracking hiệu suất. |
| **Ecom-Agent** | E-commerce Manager | Quản trị gian hàng Shopee/Lazada/TikTok Shop. |
| **Ads-Manager** | Performance Marketer | Chạy Ads, tối ưu hóa ngân sách, báo cáo ROI. |
| **Editor-Assistant** | Media Assistant | Review video, check chất lượng output, quản lý asset. |

## 3. Quy tắc Thực thi (Execution Rules)

1. **Brand Voice Adherence**: Mọi content tạo ra phải đúng tông giọng của Magic Lamp hoặc của Brand khách hàng.
2. **Data Privacy**: Dữ liệu của khách hàng A không bao giờ được lộ sang khách hàng B.
3. **Daily Sync**: Mọi Agent phải cập nhật kết quả công việc vào `Memory Tree` hàng ngày.
4. **Approval Gate**: Các kịch bản quan trọng phải qua `Creative-AI` (Lead) duyệt trước khi gửi khách.
5. **Audit Ready**: Mọi giao dịch tài chính do `ML-Finance` thực hiện phải có bằng chứng số (digital evidence).
